...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(data, dict):
        ...
        raise ValueError(f"{path.name}: config root must be a table, got {kind}")
    return data
...
def api_sections(data: dict[str, Any]) -> dict[str, dict[str, Any]]:
    """The [apis.NAME] sections of a config, or an empty dict."""
    sections = data.get("apis")
    return sections if isinstance(sections, dict) else {}


def resolve(data: dict[str, Any], env: str | None = None) -> dict[str, Any]:
...
def build_config_document(settings: dict[str, Any]) -> str:
...
def run_wizard(
    ...
    spec_loader: Callable[[str], tuple[dict[str, Any], str]] | None = None,
    ...
    if spec_loader is not None:
        try:
            _spec, base_default = spec_loader(spec_arg)
        except Exception:  # wizard degrades to a manual base URL
            base_default = ""
